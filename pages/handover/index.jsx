...
const HandoverPage = () => {
  ...
  const [currentPage, setCurrentPage] = useState(`${process.env.NEXT_PUBLIC_BACKEND_URL}/handovers?page=${pageNumber}`)
  const [disablePaginations, setDisablePaginations] = useState(false)

  const filterResult = (e) => {
    e.preventDefault()
    setCurrentPage(`${process.env.NEXT_PUBLIC_BACKEND_URL}/handovers?is_approved=${approved}&date_added_before=${endDate}&date_added_after=${startDate}`)
    setPageNumber(1)
